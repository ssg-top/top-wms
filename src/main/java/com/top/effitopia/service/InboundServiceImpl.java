...
public class InboundServiceImpl implements InboundService {
    ...
    /**
     * 특정 입고 요청 리스트를 승인하는 메서드
     * 추가적으로 입고가 승인되면 QR 코드를 생성함
     *
    ...
    public boolean approveInboundRequests(List<Integer> inboundIds) {
        ...
        if (success) {
            for (Integer inboundId : inboundIds) {
                InboundDTO inboundDTO = inboundMapper.selectOne(inboundId)
                    .map(inbound -> modelMapper.map(inbound, InboundDTO.class))
                    .orElseThrow(() -> new IllegalArgumentException("해당하는 입고요청이 존재하지 않습니다.: " + inboundId));
